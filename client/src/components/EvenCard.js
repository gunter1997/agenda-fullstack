...
const EvenCard = ({ toDo }) => {
  ...
  const markAsComplete = (e) => {
    ...
    axios.put(`/api/agendas/${toDo._id}/complete`).then((res) => {
  ...
  const markAsIncomplete = (e) => {
    ...
    axios.put(`/api/agendas/${toDo._id}/incomplete`).then((res) => {
  ...
  const deleteToDo = (e) => {
    ...
    if (window.confirm("Are you sure you want to delete this EVENT?")) {
      axios.delete(`/api/agendas/${toDo._id}`).then(() => {
        removeToDo(toDo);
      });
    }
  };
  ...
  const editToDo = (e) => {
    ...
    axios
      .put(`/api/agendas/${toDo._id}`, { content })
      .then((res) => {
        updateToDo(res.data);
        setEditing(false);
      })
